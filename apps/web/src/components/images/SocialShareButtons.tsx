...
interface SocialShareButtonsProps {
  url: string;
  title: string;
  /** When true renders a compact horizontal row; default is full-width stacked. */
  compact?: boolean;
  className?: string;
}
...
interface ShareTarget {
  ...
  /** Inline SVG path — keeps bundle small, no icon lib needed. */
...
const TARGETS: ShareTarget[] = [
  {
    label: 'X / Twitter',
    href: (url, title) =>
      `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title + ' — shared via Picflow')}`,
    icon: (
...
/**
 * Social share buttons for Twitter/X, WhatsApp, Facebook, LinkedIn.
 * Opens native share sheets in a small popup window.
 */
export function SocialShareButtons({
  ...
  const openShare = (href: string): void => {
  ...
  return (
    ...
    >
      {!compact && (
        <p className="text-caption text-text-tertiary">Share on</p>
      )}
      {TARGETS.map((t) => (
        <button
          key={t.label}
          type="button"
          onClick={() => openShare(t.href(url, title))}
          aria-label={`Share on ${t.label}`}
          className={cn(
            'inline-flex items-center gap-2 rounded-md border border-border-default px-3 text-xs font-medium text-text-secondary transition-colors duration-150',
